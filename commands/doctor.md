...
- `FAIL:missing` → print `✗ CLI installed (~/.kobiton/bin/kobiton not found)` and `    → Run /automate:setup to install the symlink (on Claude Code, restarting the session also re-creates it via the SessionStart hook). On Codex CLI or Gemini CLI, run the bundled installer directly: bash "$(find ~/.codex ~/.gemini -name install-cli.sh -path '*automate*' 2>/dev/null | head -1)".`
