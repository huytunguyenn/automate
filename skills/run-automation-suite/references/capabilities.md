...
- **Must-match** (`platformName`, `platformVersion`, `appium:udid`, `appium:deviceName`, `appium:app` / `browserName`, `appium:automationName`, `kobiton:aiToolName`): if different, show the user what will change and edit the script automatically. Device/platform fields must match the selected device/app or the Kobiton session will fail to start correctly. `kobiton:aiToolName` must reflect the *current* AI CLI running this skill — a stale value left over from a prior session run under a different CLI would mis-attribute adoption analytics. Always overwrite with the rendered value (or remove the capability entirely if the rendered output omits it, e.g. when no runtime marker matched).
